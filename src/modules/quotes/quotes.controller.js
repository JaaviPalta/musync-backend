...
export async function updateQuoteStatusController(req, res, next) {
  try {
    const quote = await updateQuoteStatus(req.user.userId, req.params.id, req.body);
    return res.status(200).json({ data: quote, message: 'Estado de la cotización actualizado correctamente' });
  } catch (error) {
    return next(error);
  }
}
